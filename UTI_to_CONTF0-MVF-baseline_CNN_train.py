...
-> this script is for training the ContF0 & MVF parameters
'''
...
for speaker in speakers:
    ...
    for train_valid in ['train', 'valid']:
        ...
        contf0_mvf[train_valid] = np.empty((n_max_ultrasound_frames, n_contf0_mvf))
        ...
        for basefile in ult_files[train_valid]:
            try:
                ...
                ultcontf0mvf_len = np.min((len(ult_data),len(contf0)))
                ult_data = ult_data[0:ultcontf0mvf_len]
                ...
                for i in range(ultcontf0mvf_len):
                    ult[train_valid][ultcontf0mvf_size[train_valid] + i] = skimage.transform.resize(ult_data[i], (n_lines, n_pixels_reduced), preserve_range=True) / 255
                
                # store contf0 and mvf in single array
                contf0_mvf[train_valid][ultcontf0mvf_size[train_valid] : ultcontf0mvf_size[train_valid] + ultcontf0mvf_len, 0] = contf0
                contf0_mvf[train_valid][ultcontf0mvf_size[train_valid] : ultcontf0mvf_size[train_valid] + ultcontf0mvf_len, 1] = mvf
                
                ultcontf0mvf_size[train_valid] += ultcontf0mvf_len
                
                print('n_frames_all: ', ultcontf0mvf_size[train_valid])


        ult[train_valid] = ult[train_valid][0 : ultcontf0mvf_size[train_valid]]
        contf0_mvf[train_valid] = contf0_mvf[train_valid][0 : ultcontf0mvf_size[train_valid]]
    ...
    for i in range(n_contf0_mvf):
        contf0_mvf_scaler = StandardScaler(with_mean=True, with_std=True)
        contf0_mvf_scalers.append(contf0_mvf_scaler)
        contf0_mvf['train'][:, i] = contf0_mvf_scalers[i].fit_transform(contf0_mvf['train'][:, i].reshape(-1, 1)).ravel()
        contf0_mvf['valid'][:, i] = contf0_mvf_scalers[i].transform(contf0_mvf['valid'][:, i].reshape(-1, 1)).ravel()
    ...
    model.add(Dropout(0.2)) 
    model.add(Dense(n_contf0_mvf,activation='linear'))

    # compile model
    # suggested lr values: 0.001, 0.003, 0.01, 0.03, 0.1, 0.3,... 
    model.compile(SGD(lr=0.01,  momentum=0.1, nesterov=True),loss='mean_squared_error', metrics=['mean_squared_error'])
    ...
    print(current_date)
    model_name = 'models/UTI_to_CONTF0-MVF_CNN-improved_' + speaker + '_' + current_date
    ...
    pickle.dump(contf0_mvf_scalers, open(model_name + '_contf0_mvf_scalers.sav', 'wb'))

    # Run training
    history = model.fit(ult['train'], contf0_mvf['train'],
                            epochs = 100, batch_size = 128, shuffle = True, verbose = 1,
                            validation_data=(ult['valid'], contf0_mvf['valid']),
